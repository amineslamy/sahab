function e(e){let n=app.utils.getApiExampleURL(),r={collectionId:e.id,collectionName:e.name},i=[{title:200,value:JSON.stringify([{status:200,body:Object.assign(r,app.utils.getDummyFieldsData(e))},{status:200,body:Object.assign(r,app.utils.getDummyFieldsData(e))}],null,2)},{title:400,value:`
            ...
            `}];return t.div({pbEvent:`apiPreviewBatch`,className:`content`},t.p(null,`Batch and transactional create/update/upsert/delete of multiple records in a single request.`),t.div({className:`alert warning`},t.p({className:`txt-bold`},`API وب دسته‌ای باید به صراحت فعال و پیکربندی شود.`,t.a({href:`#/settings`,target:`_blank`,title:`Open in new tab`,textContent:`App settings`}),`.`),t.p(null,`از آنجا که این نقطه پایانی درخواست‌ها را در یک تراکنش پایگاه داده واحد پردازش می‌کند، در صورت عدم استفاده با دقت و پیکربندی مناسب (استفاده از حداکثر پردازش و محدودیت‌های اندازه بدنه کوچک‌تر، جلوگیری از آپلود فایل‌های بزرگ در شبکه‌های S3 کند و قلاب‌های سفارشی که با APIهای خارجی کند ارتباط برقرار می‌کنند) می‌تواند عملکرد برنامه شما را کاهش دهد.`)),app.components.codeBlockTabs({className:`sdk-examples m-t-sm`,historyKey:`pbLastSDK`,tabs:[{title:`JS SDK`,language:`js`,value:`
